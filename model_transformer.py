...
class TransformerBlock(nn.Module):
    ...
    def forward(self, inputs, enc_outputs=None):
        if enc_outputs == None:
            attn_output = self.att(inputs)  # inputs bs,13,188, output bs,13,188
        else:
            attn_output = self.att(inputs, enc_outputs)
...
class Transformer(nn.Module):
    def __init__(self, num_tokens=1,
        ...
        for i in range(num_decoder_layers):
            self.decs.append(TransformerBlock(embed_dim, feat_dim, num_heads, ff_dim, rate=0.1))
